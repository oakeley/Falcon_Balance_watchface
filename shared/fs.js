...
export function readFileSync(filename, options) {
  ...
  //logger.log("readFileSync 1 --> ", filename);
  const fs_stat = statSync(filename);
  //logger.log("readFileSync 2 --> ", filename);
  if (!fs_stat) return undefined;
  //logger.log("readFileSync begin --> ", filename);
  let destination_buf = new Uint8Array(fs_stat.size);
  //打开/创建文件
  ...
  //读取结果打印
  //logger.log("readFileSync success --> ", filename);
  return content;
}
